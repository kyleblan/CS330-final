...
//Window title
const char* const WINDOW_TITLE = "7-1 FInal Project: Submission";

//Window width & height, standard monitor resolution
...
//Camera instance, set the initial position of the camera
...
glm::vec3 gSpotLightPosition(2.5f, 6.0f, -15.5f);
glm::vec3 gSpotLightScale(1.0f);

// Light color, position and scale
glm::vec3 gKeyLightColor(1.0f, 1.0f, 1.0f);
glm::vec3 gKeyLightPosition(2.5f, 4.0f, 5.5f);
...
//Initialize keyboard and mouse input functions
...
void main()
{
	...
	float spotStrength = 1.0f; // Set ambient or global lighting strength
	...
	vec3 lightDirection = normalize(lightPos - shapeColor); // Calculate distance (light direction) between light source and fragments/pixels on cube
	vec3 keyLightDirection = normalize(keyLightPos - shapeColor); // Calculate distance (light direction) between light source and fragments/pixels on cube
	...
	vec3 viewDir = normalize(viewPosition - shapeColor); // Calculate view direction
...
void UProcessInput(GLFWwindow* window)
{
	...
	// change perspective, press P to toggle perspective off, press O for on
